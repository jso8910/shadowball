...
class ShadowballSheet:
    def __init__(self):
        ...
        self.spreadsheet = self.client.open_by_key(
            "1eeSWjQR34ie2o6-CyOsUoqawhW5HmxScYZ06xxRoWGo")
    ...
    def new_game(self, away, home, day):
        template = self.spreadsheet.worksheet("Template")
        template.duplicate(insert_sheet_index=1,
                           new_sheet_name=f"{away} v {home} {day}")
    ...
    def make_guess(self, username, guess):
        ...
        if self.game == None:
            raise ValueError(
                "Current game hasn't been set. Try using (or telling your GM to use) /set_game")
        ...
        if idx == None:
            raise ValueError(
                "You aren't a Diamondback, stop using this bot smh")
        ...
        game_sheet.update("P3:R21", current_guesses,
    ...
    def finish_pitch(self, pitch, homer_diff):
        ...
        if pitch < 1 or pitch > 1000:
            raise ValueError(
                "Pitch must be a number from 1–1000. No more, no less.")
        if self.game == None:
            raise ValueError(
                "Current game hasn't been set. Try using /set_game. You should know this!")
        game_sheet = self.spreadsheet.worksheet(self.game)
        ...
        for row in guesses:
            ...
            if idx == None:
                raise ValueError(
                    f"I really don't know what went wrong but just complain to me (jso#4316). Tell me that probably it had something to do with {row[0]} not being in the leaderboard.")
            ...
            if idx == None:
                raise ValueError(
                    f"I really don't know what went wrong but just complain to me (jso#4316). Tell me that it probably had something to do with {row[0]} not being in the homer leaderboard.")
            if diff <= homer_diff:
    ...
    def get_leaderboard(self, min_pitches):
        ...
        if self.game == None:
            raise ValueError(
                "Current game hasn't been set. Try using /set_game. You should know this, you're a GM!")
        game_sheet = self.spreadsheet.worksheet(self.game)
        leaderboard = game_sheet.get("B3:D21")

        filtered_leaderboard = list(
            filter(lambda row: int(row[1]) >= min_pitches, leaderboard))
